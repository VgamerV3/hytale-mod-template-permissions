...
public final class PermissionsModPlugin extends JavaPlugin {
  ...
  private final PermissionsModTemplate service = new PermissionsModTemplate();
  private final AtomicLong heartbeatTicks = new AtomicLong();
  private final ScheduledExecutorService scheduler =
      Executors.newSingleThreadScheduledExecutor(
          runnable -> {
            Thread thread = new Thread(runnable, "hd-permissions-mod-worker");
  ...
  public PermissionsModPlugin(JavaPluginInit init) {
    super(init);
  }
  ...
  protected void setup() {
    ...
    getCommandRegistry().registerCommand(new PermissionsModStatusCommand());
    getCommandRegistry().registerCommand(new PermissionsModDemoCommand());
        ...
        scheduler.scheduleAtFixedRate(
            () -> {
              try {
                ...
                if (tick % 60 == 0) {
                  getLogger().atInfo().log("[PermissionsMod] heartbeat=%d", tick);
                }
              } catch (Exception exception) {
                lifecycle = Lifecycle.FAILED;
                service.incrementErrorCount();
                getLogger().atInfo().log("[PermissionsMod] heartbeat failed: %s", exception.getMessage());
              }
            },
            1,
            1,
  ...
  private final class PermissionsModStatusCommand extends CommandBase {
    private PermissionsModStatusCommand() {
      super("hdpermissionsmodstatus", "Shows runtime status for PermissionsModPlugin.");
  ...
  private final class PermissionsModDemoCommand extends CommandBase {
    private PermissionsModDemoCommand() {
      super("hdpermissionsmoddemo", "Runs a demo action for PermissionsModPlugin.");
    setAllowsExtraArguments(true);
      this.setPermissionGroup(GameMode.Adventure);
    }
